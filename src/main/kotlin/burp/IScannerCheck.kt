...
interface IScannerCheck {
     ...
     * @return A list of `IScanIssue` objects, or `null`
     * if no issues are identified.
     */
    fun doPassiveScan(baseRequestResponse: IHttpRequestResponse): List<IScanIssue>
     ...
     * `IScannerInsertionPoint` object provided to build scan
     ...
     * @param baseRequestResponse The base HTTP request / response that should
     * be actively scanned.
     * @param insertionPoint An `IScannerInsertionPoint` object that
    ...
    fun doActiveScan(
            ...
            insertionPoint: IScannerInsertionPoint): List<IScanIssue>
     ...
     * Scanner results. The method should return `-1` to report the
     * existing issue only, `0` to report both issues, and
     * `1` to report the new issue only.
     */
    fun consolidateDuplicateIssues(
            existingIssue: IScanIssue,
            newIssue: IScanIssue): Int
}/*
 * @(#)IScannerCheck.java
 *
